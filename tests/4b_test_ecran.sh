...
#     `Ecran/hardware/touchv2.py`, en activant l’environnement virtuel du projet puis en lançant
#     l’interface de test en mode terminal.
#
#  Entrée principale :
#     - Exécution directe :
#         ./4_test_Ecran.sh
#         -> Lance : python3 Ecran/hardware/touchv2.py
#
#  Étapes principales :
#     1) Active le venv :
...
#        - Ecran/hardware/touchv2.py doit exister, sinon erreur + deactivate + exit.
#
#     4) Lance le test TFT :
#        - python3 Ecran/hardware/touchv2.py
#
...
if [ ! -f "Ecran/hardware/touchv2.py" ]; then
    echo "❌ Fichier Ecran/hardware/touchv2.py introuvable dans le projet."
    deactivate
    exit 1
fi

# --- Lancement du GUI texte ---
echo "🖥️  Démarrage de Ecran/hardware/touchv2.py..."
python3 Ecran/hardware/touchv2.py
